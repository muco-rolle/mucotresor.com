...
Once we know the problem it's time to break it down into smaller pieces why? because a smaller thing is really easy to reason about other than something which has a lot of abstraction many stuffs so what do we need in first place? as we want to generate the content from a markdown down file to an html file we need

-   To read the content from a markdown down file
-   Parsing the content we get from that file(because they are different content we want to get them from markdown to html)
-   Generating content into html
